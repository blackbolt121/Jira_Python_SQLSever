...
IF (EXISTS(SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'dbo' AND TABLE_NAME = 'STATUS_CATEGORY'))
BEGIN
	DROP TABLE [STATUS_CATEGORY]
END
...
CREATE TABLE [STATUS](
	[ID] INTEGER PRIMARY KEY,
	[STATUS] VARCHAR(50),
	CATEGORY INT FOREIGN KEY REFERENCES STATUS_CATEGORY([CategoryID])
);
...
CREATE TABLE [ISSUES](
	...
	[TYPE] INTEGER FOREIGN KEY REFERENCES ISSUE_TYPE(ID),
