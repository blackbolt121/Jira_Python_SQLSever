...
def publishMultipleScrumReport():
    ...
    try:
        content = postOnlyJsonRequestContent(url=f"{jira_rest_api}/issue",
                                             payload=issue_payload)
        id = getProperty(content, "id")
        GenerateCsvFromQuery(
            query=
            "SELECT p.name as 'Project Name', count(b.id) '# of Scrum Boards' FROM PROJECT p, BOARD b WHERE p.id = b.projectId AND b.type = 'scrum' GROUP BY p.name HAVING count(p.id) > 1"
        )
        postAttachment(url=f"{jira_rest_api}/issue/{id}/attachments",
                       filename="report.csv",
                       filepath="./file.txt")
